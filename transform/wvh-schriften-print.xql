...
module namespace model="http://www.tei-c.org/pm/models/wvh-schriften/print";
    ...
    $input !         (
            ...
            return
                    ...
                    case element(list) return
                        if (@rend='arabic') then
                            html:list($config, ., ("tei-list1", css:map-rend-to-class(.)), ., ())
                        else
                            if (@rend='roman') then
                                html:list($config, ., css:get-rendition(., ("tei-list2", css:map-rend-to-class(.))), item, ())
                            else
                                $config?apply($config, ./node())
                    ...
                    case element(pb) return
                        html:break($config, ., ("tei-pb1", "pb-page-number", css:map-rend-to-class(.)), ., 'page', (concat('[', if(@n) then concat(@n,'] ') else '', if(@facs) then concat('@',@facs) else '')))
                    ...
                    case element(div) return
                        if (self::div) then
                            (
                                if (self::div) then
                                    html:inline($config, ., ("tei-div1", css:map-rend-to-class(.)), root(.)//facsimile/graphic)
                                else
                                    (),
                                if (self::div) then
                                    html:block($config, ., ("tei-div2", css:map-rend-to-class(.)), root (.)//sourceDesc/msDesc/msContents/summary)
                                else
                                    (),
                                if (self::div) then
                            ...
                            if (self::div) then
                                ...
                                if (self::div) then
                                    html:section($config, ., ("tei-div5", css:map-rend-to-class(.)), .)
                                else
                                    if (self::div) then
                                        html:block($config, ., ("tei-div6", css:map-rend-to-class(.)), .)
                                    else
                                        $config?apply($config, ./node())
                    ...
                    case element(title) return
                        if (@type='main') then
                            html:heading($config, ., ("tei-title1", css:map-rend-to-class(.)), ., 1)
                        else
                            if (@type='sub') then
                    ...
                    case element(sp) return
                        html:block($config, ., ("tei-sp1", css:map-rend-to-class(.)), .)
                    ...
                    case element(emp) return
                        html:inline($config, ., ("tei-emp", css:map-rend-to-class(.)), .)
                    case element(persName) return
                        printcss:alternate($config, ., ("tei-persName", css:map-rend-to-class(.)), ., ., id(@ref, doc('/db/apps/wvh-schriften/resources/registers/person.xml')))
                    case element(person) return
                        (
                            html:block($config, ., ("tei-person1", css:map-rend-to-class(.)), persName[@type="main"]),
                            html:block($config, ., ("tei-person2", css:map-rend-to-class(.)), concat(            '(* ',            substring(birth/date/@when, 9, 2), '.',             substring(birth/date/@when, 6, 2), '.',             substring(birth/date/@when, 1, 4), ' in ',             birth/placeName,             ' – † ',            substring(death/date/@when, 9, 2), '.',             substring(death/date/@when, 6, 2), '.',             substring(death/date/@when, 1, 4), ' in ',             death/placeName,             ')'          )),
                            html:block($config, ., ("tei-person3", css:map-rend-to-class(.)), note/node())
                        )
                    ...
                    case element(place) return
                        (
                            html:block($config, ., ("tei-place1", css:map-rend-to-class(.)), placeName[@type="main"]),
                            html:block($config, ., ("tei-place2", css:map-rend-to-class(.)), note/node())
                        )

                    case element(placeName) return
                        printcss:alternate($config, ., ("tei-placeName", css:map-rend-to-class(.)), ., ., id(@ref, doc('/db/apps/wvh-schriften/resources/registers/places.xml')))
                    case element(org) return
                        (
                            html:block($config, ., ("tei-org1", css:map-rend-to-class(.)), orgName[@type="main"]),
                            html:block($config, ., ("tei-org2", css:map-rend-to-class(.)), note/node())
                        )

                    case element(orgName) return
                        printcss:alternate($config, ., ("tei-orgName", css:map-rend-to-class(.)), ., ., id(@ref, doc('/db/apps/wvh-schriften/resources/registers/organizations.xml')))
