...
module namespace model="http://www.tei-c.org/pm/models/wvh-schriften/epub";
    ...
    $input !         (
            ...
            return
                    ...
                    case element(pb) return
                        epub:break($config, ., ("tei-pb1", "pb-page-number", css:map-rend-to-class(.)), ., 'page', (concat('[', if(@n) then concat(@n,'] ') else '', if(@facs) then concat('@',@facs) else '')))
                        ...
                        if (self::div) then
                            (
                                if (self::div) then
                                    ...
                                    (),
                                if (self::div) then
                                    epub:block($config, ., ("tei-div2", css:map-rend-to-class(.)), root (.)//sourceDesc/msDesc/msContents/summary)
                                else
                                    (),
                                if (self::div) then
                            ...
                            if (self::div) then
                                ...
                                if (self::div) then
                                    ...
                                    if (self::div) then
                                        epub:block($config, ., ("tei-div6", css:map-rend-to-class(.)), .)
                                    else
                                        $config?apply($config, ./node())
                    ...
                    case element(sp) return
                        epub:block($config, ., ("tei-sp1", css:map-rend-to-class(.)), .)
                    ...
                    case element(persName) return
                        epub:alternate($config, ., ("tei-persName", css:map-rend-to-class(.)), ., ., id(@ref, doc('/db/apps/wvh-schriften/resources/registers/person.xml')))
                    case element(person) return
                        (
                            epub:block($config, ., ("tei-person1", css:map-rend-to-class(.)), persName[@type="main"]),
                            epub:block($config, ., ("tei-person2", css:map-rend-to-class(.)), concat(            '(* ',            substring(birth/date/@when, 9, 2), '.',             substring(birth/date/@when, 6, 2), '.',             substring(birth/date/@when, 1, 4), ' in ',             birth/placeName,             ' – † ',            substring(death/date/@when, 9, 2), '.',             substring(death/date/@when, 6, 2), '.',             substring(death/date/@when, 1, 4), ' in ',             death/placeName,             ')'          )),
                            epub:block($config, ., ("tei-person3", css:map-rend-to-class(.)), note/node())
                        )
                    ...
                    case element(place) return
                        (
                            epub:block($config, ., ("tei-place1", css:map-rend-to-class(.)), placeName[@type="main"]),
                            epub:block($config, ., ("tei-place2", css:map-rend-to-class(.)), note/node())
                        )

                    case element(placeName) return
                        epub:alternate($config, ., ("tei-placeName", css:map-rend-to-class(.)), ., ., id(@ref, doc('/db/apps/wvh-schriften/resources/registers/places.xml')))
                    case element(org) return
                        (
                            epub:block($config, ., ("tei-org1", css:map-rend-to-class(.)), orgName[@type="main"]),
                            epub:block($config, ., ("tei-org2", css:map-rend-to-class(.)), note/node())
                        )

                    case element(orgName) return
                        epub:alternate($config, ., ("tei-orgName", css:map-rend-to-class(.)), ., ., id(@ref, doc('/db/apps/wvh-schriften/resources/registers/organizations.xml')))
