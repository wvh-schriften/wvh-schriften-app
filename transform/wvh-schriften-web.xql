(:~
    ...
    ODD: /db/apps/wvh-schriften/resources/odd/wvh-schriften.odd
 :)
xquery version "3.1";

module namespace model="http://www.tei-c.org/pm/models/wvh-schriften/web";
    ...
    $input !         (
            ...
            return
                    ...
                    case element(p) return
                        if (ancestor::sp) then
                            ...
                            if (@rend='center') then
                                ...
                                html:paragraph($config, ., css:get-rendition(., ("tei-p3", css:map-rend-to-class(.))), .)                                => model:map($node, $trackIds)
                    case element(list) return
                        if (@rend='arabic') then
                            html:list($config, ., ("tei-list1", css:map-rend-to-class(.)), ., ())                            => model:map($node, $trackIds)
                        else
                            if (@rend='roman') then
                                html:list($config, ., css:get-rendition(., ("tei-list2", css:map-rend-to-class(.))), item, ())                                => model:map($node, $trackIds)
                            else
                                $config?apply($config, ./node())
                    ...
                    case element(pb) return
                        html:break($config, ., ("tei-pb1", "pb-page-number", css:map-rend-to-class(.)), ., 'page', (concat('[', if(@n) then concat(@n,'] ') else '', if(@facs) then concat('@',@facs) else '')))                        => model:map($node, $trackIds)
                    ...
                    case element(div) return
                        if (self::div) then
                            (
                                if (self::div) then
                                    html:inline($config, ., ("tei-div1", css:map-rend-to-class(.)), root(.)//facsimile/graphic)                                    => model:map($node, $trackIds)
                                else
                                    (),
                                if (self::div) then
                                    html:block($config, ., ("tei-div2", css:map-rend-to-class(.)), root (.)//sourceDesc/msDesc/msContents/summary)                                    => model:map($node, $trackIds)
                                else
                                    (),
                                if (self::div) then
                            ...
                            if (self::div) then
                                ...
                                if (self::div) then
                                    html:section($config, ., ("tei-div5", css:map-rend-to-class(.)), .)                                    => model:map($node, $trackIds)
                                else
                                    if (self::div) then
                                        html:block($config, ., ("tei-div6", css:map-rend-to-class(.)), .)                                        => model:map($node, $trackIds)
                                    else
                                        $config?apply($config, ./node())
                    ...
                    case element(title) return
                        if (@type='main') then
                            html:heading($config, ., ("tei-title1", css:map-rend-to-class(.)), ., 1)                            => model:map($node, $trackIds)
                        else
                            if (@type='sub') then
                    ...
                    case element(date) return
                        if (@when) then
                            html:inline($config, ., ("tei-date1", css:map-rend-to-class(.)), .)                            => model:map($node, $trackIds)
                        else
                    ...
                    case element(sp) return
                        html:block($config, ., ("tei-sp1", css:map-rend-to-class(.)), .)                        => model:map($node, $trackIds)
                    ...
                    case element(emp) return
                        html:inline($config, ., ("tei-emp", css:map-rend-to-class(.)), .)                        => model:map($node, $trackIds)
                    case element(persName) return
                        html:alternate($config, ., ("tei-persName", css:map-rend-to-class(.)), ., ., id(@ref, doc('/db/apps/wvh-schriften/resources/registers/person.xml')), map {})                        => model:map($node, $trackIds)
                    case element(person) return
                        (
                            html:block($config, ., ("tei-person1", css:map-rend-to-class(.)), persName[@type="main"])                            => model:map($node, $trackIds),
                            html:block($config, ., ("tei-person2", css:map-rend-to-class(.)), concat(            '(* ',            substring(birth/date/@when, 9, 2), '.',             substring(birth/date/@when, 6, 2), '.',             substring(birth/date/@when, 1, 4), ' in ',             birth/placeName,             ' – † ',            substring(death/date/@when, 9, 2), '.',             substring(death/date/@when, 6, 2), '.',             substring(death/date/@when, 1, 4), ' in ',             death/placeName,             ')'          ))                            => model:map($node, $trackIds),
                            html:block($config, ., ("tei-person3", css:map-rend-to-class(.)), note/node())                            => model:map($node, $trackIds)
                        )
                    ...
                    case element(place) return
                        (
                            html:block($config, ., ("tei-place1", css:map-rend-to-class(.)), placeName[@type="main"])                            => model:map($node, $trackIds),
                            html:block($config, ., ("tei-place2", css:map-rend-to-class(.)), note/node())                            => model:map($node, $trackIds)
                        )

                    case element(placeName) return
                        html:alternate($config, ., ("tei-placeName", css:map-rend-to-class(.)), ., ., id(@ref, doc('/db/apps/wvh-schriften/resources/registers/places.xml')), map {})                        => model:map($node, $trackIds)
                    case element(org) return
                        (
                            html:block($config, ., ("tei-org1", css:map-rend-to-class(.)), orgName[@type="main"])                            => model:map($node, $trackIds),
                            html:block($config, ., ("tei-org2", css:map-rend-to-class(.)), note/node())                            => model:map($node, $trackIds)
                        )

                    case element(orgName) return
                        html:alternate($config, ., ("tei-orgName", css:map-rend-to-class(.)), ., ., id(@ref, doc('/db/apps/wvh-schriften/resources/registers/organizations.xml')), map {})                        => model:map($node, $trackIds)
